...
btnUpdateProductIn.addEventListener("click", (e) => {
    ...
    // Lấy ảnh hiện tại từ thẻ img
    let imgProductCur = document.querySelector(".upload-image-preview").src;
    console.log("Đường dẫn hình ảnh hiện tại:", imgProductCur);
    ...
    // Lấy các giá trị hiện tại từ input
    ...
    // So sánh nếu có bất kỳ thay đổi nào trong dữ liệu
    if (
        (imgProductCur.startsWith("data:") && imgProductCur !== imgProduct) || // Kiểm tra URL tạm của ảnh
        titleProductCur !== titleProduct ||
        curProductCur != curProduct || // Sử dụng != để so sánh số và chuỗi số
        ...
        let productadd = {
            ...
            price: parseInt(curProductCur), // Chuyển đổi chuỗi giá sang số
            desc: descProductCur,
            status: 1,
        };
...
function uploadImage(el) {
    let file = el.files[0]; // Lấy file từ input
    if (file) {
        let reader = new FileReader();
        reader.onload = function(e) {
            document.querySelector(".upload-image-preview").setAttribute("src", e.target.result); // Đặt URL từ FileReader
        };
        reader.readAsDataURL(file); // Đọc file và tạo URL tạm
    } else {
        document.querySelector(".upload-image-preview").setAttribute("src", ""); // Đặt lại nếu không có file
    }
}
